...
import { CheckCircle2, Home, MapPin, Navigation, QrCode, Truck } from "lucide-react-native";
        ...
        <View className="flex-1 bg-[#195FA0]">
            <ScrollView contentContainerStyle={{ paddingBottom: 40, flexGrow: 1 }} showsVerticalScrollIndicator={false}>
                ...
                <Center className="pt-16 pb-8">
                    ...
                    <Heading className="text-white font-bold text-2xl">Tudo Certo!</Heading>
                ...
                <View className="mx-6 bg-white rounded-3xl overflow-hidden shadow-2xl shadow-black/50">
                    ...
                    <View className="p-6 bg-gray-50 border-b border-dashed border-gray-300">
                        <HStack className="justify-between items-center mb-6">
                            <VStack>
                                <Text className="text-xs text-gray-400 font-bold uppercase">Data</Text>
                                <Text className="text-gray-800 font-bold text-lg capitalize">{dataFormatada}</Text>
                            </VStack>
                            <VStack className="items-end">
                                <Text className="text-xs text-gray-400 font-bold uppercase">Horário</Text>
                        ...
                        <HStack space="md" className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm items-center">
                            <View className="bg-blue-50 p-3 rounded-lg">
                                <Icon as={MapPin} className="text-[#195FA0]" />
                            </View>
                            <VStack className="flex-1">
                                <Text className="text-xs text-gray-400 font-bold">Local de Descarga</Text>
                                <Text className="text-gray-800 font-bold text-md" numberOfLines={1}>
                    ...
                    <View className="p-6">
                        <VStack space="md">
                            <HStack className="justify-between">
                                <VStack>
                                    <Text className="text-xs text-gray-400 mb-1">Veículo (Placa)</Text>
                                    <HStack space="xs" className="items-center">
                                        <Icon as={Truck} size="xs" className="text-gray-600" />
                                        <Text className="font-bold text-gray-800 text-lg">{placaVeiculo || "---"}</Text>
                                    </HStack>
                                </VStack>
                                <VStack className="items-end">
                                    <Text className="text-xs text-gray-400 mb-1">Carga</Text>
                                    <Text className="font-bold text-gray-800 text-lg">
                                        {notaEmConferencia?.pesoBruto ? `${notaEmConferencia.pesoBruto} kg` : "N/A"}
                                    </Text>
                                </VStack>
                            </HStack>
                        </VStack>
                ...
                <VStack space="md" className="px-6 mt-8">
                    ...
                    >
                        <ButtonText className="text-[#195FA0] font-bold">Voltar ao Início</ButtonText>
